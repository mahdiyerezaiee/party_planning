...
/**
 * This component renders a button with a back arrow icon to navigate back to the previous page
 *
 * @constructor
 */
const ButtonBackTodo:React.FC= () => {
    /** Retrieve the dispatch function and page state from Redux store **/
    const dispatch = useAppDispatch()
    const state = useAppSelector(state => state.page)
    let {page} = state

    /** dispatches an action to decrease the page state by 1 **/
    const clickHandler = () => {
